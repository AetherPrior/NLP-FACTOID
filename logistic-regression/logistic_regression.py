...
scores = cross_val_score(clf, X_concat_train, y_tr, cv=5)
...
scores = cross_val_score(clf, X_add_train, y_add_tr, cv=5)
print("Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))
...
scores = cross_val_score(clf, X_average_train, y_avg_tr, cv=5)
print("Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))
